...
export const createAuction = async (req, res) => {
  try {
    ...
    const parsedDuration = duration ? parseInt(duration) : 60; // default 1 hour
    console.log("---- AUCTION CREATION TRACE ----");
    console.log("Received duration:", duration, "Parsed:", parsedDuration);
    if (parsedDuration < 1 || parsedDuration > 1440) {
      return res.status(400).json({ message: "Duration must be between 1 and 1440 minutes (24 hours)" });
    }
...
export const auctionById = async (req, res) => {
  try {
    ...
    // Winner logic relies perfectly on the cron job now. No need for fetching auto-trigger.
