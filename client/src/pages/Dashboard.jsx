  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between bg-gradient-to-r from-indigo-900 to-indigo-700 rounded-3xl p-8 mb-10 shadow-2xl overflow-hidden relative">
          ...
          <div className="relative z-10 text-white mb-6 md:mb-0">
            ...
            <p className="text-indigo-100 text-lg opacity-90 max-w-lg">
              Manage your live auctions, view massive earnings, and place bids on the world's finest marketplace.
            </p>
          </div>
          <Link
            to="/create"
            className="relative z-10 hidden sm:inline-flex items-center gap-2 bg-indigo-400 text-indigo-950 font-bold px-6 py-3.5 rounded-2xl hover:bg-white hover:scale-105 active:scale-95 transition-all shadow-[0_0_20px_rgba(129,140,248,0.4)]"
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" />
            </svg>
            Create Infinite Auction
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-16">
          {statConfig.map((stat) => (
            <div
              key={stat.key}
              className="bg-white rounded-3xl border border-gray-100 shadow-xl shadow-indigo-100/40 p-6 flex items-center gap-5 hover:-translate-y-1 transition-all duration-300"
            >
              <div className={`${stat.bg} p-4 rounded-2xl shadow-inner`}>{stat.icon}</div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">
                  {stat.label}
                </p>
                <p className={`text-4xl font-black ${stat.color} tabular-nums tracking-tighter`}>
                  {data[stat.key]}
                </p>
              </div>
            </div>
